...
class ComplexTests(unittest.TestCase):
    def setUp(self):
        ...
        self.complexes['C2'] = Complex('C2', [self.strands['OP'], self.strands['PS']], [
    ...
    def testConstructor(self):
        assert self.complexes['C1'].strands == [
            self.strands['OP'], self.strands['PS']]
        assert self.complexes['C1'].structure == [[(1, 2), (1, 1), (1, 0), None], [
            (0, 2), (0, 1), (0, 0), None, None]]

        assert self.complexes['I1'].strands == [
            self.strands['BS'], self.strands['SP'], self.strands['Cat']]
        assert self.complexes['I1'].structure == [
            [(2, 1), (1, 1), (1, 0), None, None, None], [(0, 2), (0, 1)], [None, (0, 0)]]
    ...
    def testHash(self):
        ...
        assert hash(self.complexes['C1']) == hash(
            copy.deepcopy(self.complexes['C1']))
    ...
    def testAvailableDomains2(self):
        ...
        three_arm_nodal_enum = input_enum(
            'tests/files/examples/3-arm-junction.enum')
    ...
    def testRotateStrands(self):
        ...
        assert c3.strands == [self.strands['Cat'],
                              self.strands['BS'], self.strands['SP']]
        assert c3.structure == [
            [None, (1, 0)], [(0, 1), (2, 1), (2, 0), None, None, None], [(1, 2), (1, 1)]]
    ...
    def testRotateStrands2(self):
        from peppercornenumerator.input import input_enum
        self.biggate_enum = input_enum(
            'tests/files/examples/sarma2010/biggate.in')
        (domains, strands, complexes) = index_parts(self.biggate_enum)

        c16 = Complex('16', [strands['a1'], strands['a1'],
                             strands['b1']], parse_dot_paren('(..+(((+))))'))
        c38 = Complex('38', [strands['a1'], strands['b1'],
                             strands['a1']], parse_dot_paren('(((+)))(+)..'))
        assert c16 == c38

    def testRotateLocation(self):
        i1 = self.complexes['I1']
        c3 = Complex('C3', [self.strands['SP'], self.strands['Cat'], self.strands['BS']], [
                     [(2, 2), (2, 1)], [None, (2, 0)], [(1, 1), (0, 1), (0, 0), None, None, None]])
        assert c3.strands == [self.strands['BS'],
                              self.strands['SP'], self.strands['Cat']]
        assert c3.structure == [[(2, 1), (1, 1), (1, 0), None, None, None], [
            (0, 2), (0, 1)], [None, (0, 0)]]
...
class RestingStateTests(unittest.TestCase):
    ...
    def testConstructor(self):
        ...
        comp = sorted(
            [self.complexes['C1'], self.complexes['Cat'], self.complexes['I1']])
    ...
    def testRepr(self):
        print repr(self.rs)
        assert repr(self.rs) == "RestingState(\"RS1\", %s)" % str(
            self.rs.complexes)
