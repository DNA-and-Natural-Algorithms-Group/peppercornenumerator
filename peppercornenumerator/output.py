...
def write_kernel(enumerator, filename, 
        output_condensed = False, output_rates = True, k_fast = 0.):

    """Write the contents of :obj:`Enumeraotr()` into a PIL file -- KERNEL notation).
    ...
    for dom in utils.natural_sort(enumerator.domains):
        if dom.is_complement :
            continue
        pil.write("length {:s} = {:d}\n".format(dom.name, len(dom)))
        #pil.write("sequence {:s} = {:s}\n".format(v.name, ''.join(v.sequence)))

    # Print Resting-state Complexes
    pil.write("\n# Resting-state Complexes \n")
    for cplx in utils.natural_sort(enumerator.resting_complexes):
    ...
    if output_condensed :
        ...
        pil.write("\n# Resting-state sets \n")
        #for resting in sorted(enumerator.resting_states, key=lambda x: x.canonical_name):
        ...
        pil.write("\n# Transient Complexes \n")
        for cplx in utils.natural_sort(enumerator.transient_complexes):
            pil.write("{:s} = {:s}\n".format(cplx.name, cplx.kernel_string))
...
def output_kernel(enumerator, filename, output_condensed=False,
    ...
    def write_complex(output_file, complex):
        ...
        if complex.concentration is not None:
            params = "[@ %g %sM] " % utils.format_si(complex.concentration)
        output_file.write(str(complex) + params + " = " +
                          complex.kernel_string() + "\n")
...
def output_json(enumerator, filename, output_condensed=False,
    ...
    object_out['resting_complexes'] = map(
        serializeComplex, enumerator.resting_complexes)
    object_out['transient_complexes'] = map(
        serializeComplex, enumerator.transient_complexes)
    object_out['resting_states'] = map(
        serializeRestingState, enumerator.resting_states)
    object_out['initial_complexes'] = map(
        serializeComplex, enumerator.initial_complexes)
    object_out['reactions'] = map(serializeReaction, enumerator.reactions)

    if output_condensed:
        condensed = condense_resting_states(enumerator, **condense_options)
        object_out['condensed_reactions'] = map(
            serializeReaction, condensed['reactions'])
...
def output_case(enumerator, filename, output_condensed=False,
    ...
    for complex in utils.natural_sort(enumerator.complexes):
        strands = ", ".join("strands['%s']" %
                            strand.name for strand in complex.strands)
        lines.append(tab(2) + "'%s' : Complex('%s', [%s], %r)" % (
    ...
    for reaction in utils.natural_sort(enumerator.reactions):
        ...
        lines.append(
            ...
            (reaction.rtype, reactants, products))
    of.write(",\n".join(lines) + "\n")
    of.write(tab(1) + "}\n")
